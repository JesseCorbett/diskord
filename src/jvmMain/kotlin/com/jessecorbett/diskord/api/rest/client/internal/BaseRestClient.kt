...
actual open class BaseRestClient {
    ...
    protected actual suspend fun getRequest(url: String, headers: Map<String, String>) =
        request(commonRequest(url, headers).get().build())

    protected actual suspend fun postRequest(url: String, jsonBody: String?, headers: Map<String, String>) =
        request(commonRequest(url, headers).post(jsonBody).build())

    protected actual suspend fun putRequest(url: String, jsonBody: String?, headers: Map<String, String>) =
        request(commonRequest(url, headers).put(jsonBody).build())

    protected actual suspend fun patchRequest(url: String, jsonBody: String?, headers: Map<String, String>) =
        request(commonRequest(url, headers).patch(jsonBody).build())

    protected actual suspend fun deleteRequest(url: String, headers: Map<String, String>) =
        request(commonRequest(url, headers).delete().build())
    ...
    private fun jsonBody(jsonBody: String?) =
        RequestBody.create(MediaType.get("application/json; charset=utf-8"), jsonBody ?: "")
